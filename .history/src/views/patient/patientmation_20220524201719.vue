...
<script>
import { getPatientList, getpationconsult, addconsult ,getAllSameData0,getAllSameData1,getAllSameData2} from "@/api/patient";
export default {
  name: "patientmation",
  data() {
    return {
  ...
  methods: {
    // 拿到就诊基本数据表数据(就诊地址)
    async getAllSameData() {
      await getAllSameData().then((res) => {
        console.log("获取单一类型的全部数据", res.data);
