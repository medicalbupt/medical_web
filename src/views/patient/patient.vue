<template>
  <div id="patient-manager">
    <el-card>
      ...
      <el-table :data="patientData.patientList" border stripe style="width: 100%">
        ...
        <el-table-column prop="patientName" label="患者姓名">
          <template slot-scope="scope">
            <router-link :to="{
              ...
              <el-button type="text" size="medium">{{scope.row.patientName}}</el-button>
            </router-link>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="280">
          <template slot-scope="scope">
            <el-button type="warning" size="small" @click="gotoDetail(scope.row.id, patientData.total)">详情</el-button>
            <el-button type="primary" size="small" @click="showpatientEditDialog(scope.row.id)">编辑</el-button>
            <el-button type="danger" size="small" @click="removePatientById(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->

      <el-pagination @current-change="handleCurrentChange" :current-page="queryInfo.currentPage"
        @size-change="handleSizeChange" :page-size="queryInfo.pageSize" :page-sizes="[10, 20, 50]"
        layout="total,sizes, prev, pager, next, jumper" :total="this.patientData.total">
      </el-pagination>
    </el-card>
    ...
    <!-- 添加患者对话框 -->
    <el-dialog title="添加患者" :visible.sync="addDialogVisible" top="6vh" width="800px">
      <!-- @close="addDislogClosed" -->
      <!-- 内容主题区域 -->
      <div class="dialog-content">
        <el-form class="medical-form" label-width="80px" label-position="top" size="medium" ref="addFormRef" :model="addForm">
          <el-row>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <div class="grid-content ">
                <el-form-item label="身高">
                  <el-input class="input-style" type="number" v-model="addForm.height" placeholder="单位：cm"></el-input>
                </el-form-item>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="grid-content ">
                <el-form-item label="体重">
                  <el-input class="input-style" type="number" v-model="addForm.weight" placeholder="单位：kg"></el-input>
                </el-form-item>
              </div>
            </el-col>
            <el-col :span="12">
          ...
          <el-form-item label="个人史">
            <el-row>
              <el-col :span="8">
                ...
                <el-input-number v-model="addForm.personalHistory.smoke.amount" :max="50" :min="0" controls-position="right"></el-input-number>
              </el-col>
              <el-col :span="8">
                <div class="block-title">啤酒数量（瓶/天）</div>
                <el-input-number v-model="addForm.personalHistory.beer.amount" :max="50" :min="0" controls-position="right"></el-input-number>
              </el-col>
              <el-col :span="8">
                <div class="block-title">白酒数量（两/天）</div>
                <el-input-number v-model="addForm.personalHistory.whiteWine.amount" :max="50" :min="0" controls-position="right"></el-input-number>
              </el-col>
            </el-row>
          </el-form-item>
          ...
          <el-form-item label="过敏史">
            <el-input :rows="5" placeholder="请输入内容" auto-complete="" v-model="addForm.allergyHistory" type="textarea">
            </el-input>
          </el-form-item>
    ...
    <el-dialog top="6vh" title="修改患者信息" :visible.sync="editDialogVisible" width="800px" @close="editDialogClosed">
      <div class="dialog-content">
        <el-form class="medical-form" label-width="80px" label-position="top" size="medium" :model="patinenteditform" ref="editFormRef">
        <el-row>
          <el-col :span="12">
            <div class="grid-content ">
              <el-form-item class="input-style" label="患者ID">
          ...
          <el-col :span="12">
            <div class="grid-content ">
              <el-form-item class="input-style" label="身高">
                <el-input type="number" v-model="patinenteditform.height" placeholder="单位：cm"></el-input>
              </el-form-item>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="grid-content ">
              <el-form-item class="input-style" label="体重">
                <el-input type="number" v-model="patinenteditform.weight" placeholder="单位：kg"></el-input>
              </el-form-item>
            </div>
          </el-col>
        </el-row>
        <el-form-item label="主述">
          <el-input type="textarea" :rows="5" placeholder="请输入内容" v-model="patinenteditform.mainComplaint"></el-input>
        </el-form-item>
        <!-- <span style="font-weight: bold">现病史</span> -->
        <el-form-item label="现病史">
          <el-card shadow="never">
            <span class="block-title">西医诊断及病程</span>
            <el-radio-group v-model="patinenteditform.curMedicalRecord.Westernmedicine.list">
              <el-radio v-for="item in WesternmedicineOptions" :label="item.id" :key="item.id">{{item.dataName}}
              </el-radio>
            </el-radio-group>
            <span class="block-title">确诊时间</span>
            <div>
              <el-date-picker class="input-style" v-model="patinenteditform.curMedicalRecord.confirmTime.time"
                type="date" placeholder="选择日期">
              </el-date-picker>
            </div>
            <span class="block-title">糖尿病并发症</span>
            <el-checkbox-group v-model="patinenteditform.curMedicalRecord.DMcomplications.list">
              <el-checkbox v-for="item in DMcomplicationsOptions" :key="item.id" :label="item.id">{{item.dataName}}
              </el-checkbox>
            </el-checkbox-group>
            <span class="block-title">慢性肾脏病病因</span>
            <el-checkbox-group v-model="patinenteditform.curMedicalRecord.CKDreason.list">
              <el-checkbox v-for="item in CKDreasonOptions" :label="item.id" :key="item.id">{{item.dataName}}
              </el-checkbox>
            </el-checkbox-group>
          </el-card>
        ...
        <el-form-item label="个人史">
          <el-row>
            <el-col :span="8">
              <div class="block-title">吸烟数量（支/天）</div>
              <el-input-number v-model="patinenteditform.personalHistory.smoke.amount" :max="50" :min="0" controls-position="right"></el-input-number>
            </el-col>
            <el-col :span="8">
              <div class="block-title">啤酒数量（瓶/天）</div>
              <el-input-number v-model="patinenteditform.personalHistory.beer.amount" :max="50" :min="0" controls-position="right"></el-input-number>
            </el-col>
            <el-col :span="8">
              <div class="block-title">白酒数量（两/天）</div>
              <el-input-number v-model="patinenteditform.personalHistory.whiteWine.amount" :max="50" :min="0" controls-position="right"></el-input-number>
            </el-col>
          </el-row>
        </el-form-item>
        <el-form-item label="家族史">
        ...
        <el-form-item label="过敏史">
          <el-input :rows="5" placeholder="请输入内容" auto-complete="" v-model="patinenteditform.allergyHistory"
